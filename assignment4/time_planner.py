...
def extract_events(table: bs4.element.Tag) -> pd.DataFrame:
    ...
    for tr in rows:
        ...
        row = []
        for cell in cells:
            colspan_pat = re.findall(r'colspan="(\d)"', str(cell))
            rowspan_pat = re.findall(r'rowspan="(\d)"', str(cell))
            if len(colspan_pat) > 0:
                colspan = int(colspan_pat[0])
            else:
                colspan = 1 
            if len(rowspan_pat) > 0:
                rowspan = int(rowspan_pat[0])
            else:
                rowspan = 1
            
            text = strip_text(cell.text)
            
            if colspan == len(labels):
                break
            row.append(
                TableEntry(
                    text=text,
    ...
    for i in range(len(all_data)):
        for name in wanted:
            if len(all_data[i]) > 1:
                dict[name].append(all_data[i][labels.index(name)])
                
    df = df = pd.DataFrame(data=dict)
...
def render_schedule(data: pd.DataFrame) -> str:
    ...
    data["Type"] = data["Type"].map(expand_event_type)
